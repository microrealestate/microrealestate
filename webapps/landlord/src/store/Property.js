...
export default class Property {
  ...
  *fetch() {
    try {
      const response = yield apiClient.get('/properties');
  ...
  *fetchOne(propertyId) {
    try {
      const response = yield apiClient.get(`/properties/${propertyId}`);
  ...
  *create(property) {
    try {
      const response = yield apiClient.post('/properties', property);
  ...
  *delete(ids) {
    try {
      yield apiClient.delete(`/properties/${ids.join(',')}`);
      this.items = this.items.filter((property) => !ids.includes(property._id));
